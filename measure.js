...
async function test(height) {
  ...
  const result = await page.evaluate(() => {
    ...
    const hasDash = document.body.innerText.includes('DASHBOARD OVERVIEW');
    return { hasDash, scrollers: scrollers.slice(0,3) };
  });
...
(async () => {
  for (const h of [900, 850, 820, 800, 780, 768, 750]) {
    await test(h);
  }
})();
